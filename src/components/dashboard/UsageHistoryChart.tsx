...
export function UsageHistoryChart({ history }: { history: UsageHistory }) {
  ...
  return (
    <Card className="p-6">
      <p className="text-xs font-medium text-brand-muted uppercase tracking-wider mb-6">Usage history</p>
        ...
        {history.map((entry) => {
          ...
          return (
            <div key={entry.month} className="flex flex-col items-center gap-1.5 flex-1">
              <span className="text-[10px] text-brand-muted">{entry.usedGB}GB</span>
              <div className="w-full flex items-end justify-center h-24">
                <div
                  className={`w-7 rounded-t transition-opacity hover:opacity-80 ${overCap ? 'bg-brand-purple-light' : 'bg-brand-purple'}`}
                  style={{ height: `${heightPct}%` }}
                  title={`${entry.month}: ${entry.usedGB}GB of ${entry.totalGB}GB`}
                />
              </div>
              <span className="text-[10px] text-brand-muted">{entry.month.split(' ')[0]}</span>
            </div>
          )
        })}
      </div>
